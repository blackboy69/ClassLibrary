...
sub new {
    ...
    if ( !$this->{connector}->{handle} ) {    # need to make connection
        $dsn =
"DBI:mysql:$this->{connector}->{path}:$this->{connector}->{host}:3306";
        $this->{connector}->{handle} = DBI->connect(
            $dsn,
            $this->{connector}->{user},
            $this->{connector}->{pass},
          ...
          or die
"! Cannot connect to DB $this->{connector}->{name} server: $DBI::errstr\n";
    }
...
sub query {
    ...
    $this->reconnect;
    $cx = $this->{connector};
    return $cx->{handle}->do($query)
      if ( $query !~ /^\s*\b(describe|select|show)\b/i );
...
sub selectTable {
    ...
    $this->loadTables($database)
      unless ( defined $cx->{databases}->{$database} );
...
sub fetch {
    my ( $this, $table, $key ) = @_;
    my (
        $cx,     $tab,    $fx,    @fields, $property,
        $format, $column, $query, $sth,    $record
    );
    $this->reconnect;
    $cx = $this->{connector};
...
sub store {
    ...
    else { return $this->writeRecord( $table, $record ) }
}
...
sub caching {
    ...
    elsif ( defined $this->{cache} ) { undef( $this->{cache} ); }
    else                             { $this->{cache} = {}; }
}
...
sub fetch {
    ...
    for (
        @fieldlist = (), $fx = 0 ;
        $fx < scalar @{ $this->{properties} } ;
        ++$fx
      )
    {
        push @fieldlist,
          (
            $this->{structure}->{formats}->[$fx] =~ /bit/i
    ...
    elsif ( $this->{properties}->[0] =~ /int/i
        and $this->{properties}->[1] !~ /int/i
        and $keys[0] =~ /[^0-9]/ )
    {
...
sub deleteRecord {
    ...
    undef( $this->{cache}->{$key} )
      if ( defined( $this->{cache} ) and $result );
    return $result;
}
...
#
@daysInMonth =
  ( 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );    # days per cal. month
@monthNames = (
...
sub move {
    ...
    elsif ( $vector =~ /^\s*([0-9-]+)\s*([A-Za-z]+)\s*$/ )
    {                                          # nnn minutes|hours|etc.
        $quantity = $1;
        $unit     = lc $2;
...
sub internal {
    ...
    $result .=
      sprintf( ' %02d:%02d:%02d', $this->{hour}, $this->{min}, $this->{sec} );
    return $result;
}
...
sub datestamp {
    ...
    return sprintf(
        ...
        ( $this->{year} + 1900 ),
        ( $this->{mon} + 1 ),
        $this->{mday}, $this->{hour}, $this->{min}, $this->{sec}
    );
}
...
sub validate ($) {
    ...
    if ( $datestring =~
        /(\d{4})\D(\d{1,2})\D(\d{1,2})\s+(\d{1,2})\D(\d{1,2})\D(\d{1,2})/ )
    {
