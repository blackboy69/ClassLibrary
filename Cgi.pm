...
#        |___/       |_|
#	Cgi - Common Gateway Interface class
#	June, 2013 by Harley H. Puthuff
#	Copyright 2005-13, Your Showcase
#
package Cgi;
...
##
#	Export file as html "attached file"
#
#	@param string $filename			: full path/filename to export
#	@param string $mimetype			: (optional) MIME type of file
#	@return int						: size of exported file
#
sub export {
	my ($this,$fullname,$mimetype) = @_;
	my $filesize = -s $fullname;
	my ($path,$filename);
	if ($fullname !~ m|/|)
		{$path = ""; $filename = $fullname;}
	else
		{($path,$filename) = $fullname =~ m|^(.*[/\\])([^/\\]+?)$|;}
	if (! $mimetype) {
		if ($filename =~ /\.csv$/i)		{$mimetype = "application/vnd.ms-excel";}
		elsif ($filename =~ /.xml$/i)	{$mimetype = "text/xml";}
		else							{$mimetype = "text/plain";}
		}
	put qq|Content-Type: $mimetype|;
	put qq|Content-Length: $filesize|;
	put qq|Content-Disposition: attachment; filename="$filename"\n|;
	open SOURCE,"<$fullname" or return 0;
	print STDOUT while (<SOURCE>);
	close SOURCE;
	return $filesize;
	}
